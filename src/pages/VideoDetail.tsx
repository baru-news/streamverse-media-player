...
import { ThumbsUp, Share2, Download, Eye, Calendar, User, FileText } from "lucide-react";
...
const VideoDetail = () => {
  ...
  const loadLikesCount = async (videoId: string) => {
    try {
      const { count } = await supabase.from('video_likes').select('*', {
        count: 'exact',
        head: true
      }).eq('video_id', videoId);
  ...
  const loadVideoHashtags = async (videoId: string) => {
    try {
      const { data } = await supabase.from('video_hashtags').select('hashtags!inner(id, name, color)').eq('video_id', videoId);
  ...
  const checkLikeStatus = async () => {
    ...
    try {
      const { data } = await supabase.from('video_likes').select('id').eq('user_id', user.id).eq('video_id', video.id).maybeSingle();
      setIsLiked(!!data);
    } catch (error) {
  ...
  const checkSubscriptionStatus = async () => {
    if (!user || !video) return;
    try {
      const { data } = await supabase.from('user_subscriptions').select('id').eq('subscriber_id', user.id).eq('creator_name', video.creator).maybeSingle();
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        ...
        if (error.name !== 'AbortError') { // AbortError means user cancelled, which is fine
          console.error('Error sharing:', error);
          toast({
            title: "Error",
            description: "Gagal membagikan. Link akan disalin.",
            variant: "destructive"
          });
        }
